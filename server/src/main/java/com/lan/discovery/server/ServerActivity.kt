...
//        val missing = permissions.filterNot { hasPermission(it) }
        ...
        return if (missing.isEmpty()) {
            ...
            Log.e(TAG, "ensureHotspotPermissions: 缺少权限 $missing")
            false
        }
    }
    ...
    private suspend fun startHotspot() =
        withContext(Dispatchers.Main.immediate) {
            ...
            try {
                ...
                wifiConfig.SSID = "AndroidAP3568" // 设置自定义 SSID
                wifiConfig.preSharedKey = "12345678" // 设置密码

                // 构建 SoftApConfiguration
                val softApConfig = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA) {
                    SoftApConfiguration.Builder()
//                        .setChannels()

                        //                    .setChannels("AndroidAP3568")
                        //                    .setWpa2Passphrase("StrongPass1234", SoftApConfiguration.SECURITY_TYPE_WPA2_PSK)
                        //                    .setBand(SoftApConfiguration.BAND_2GHZ) // 或 BAND_5GHZ / BAND_ANY
                        //                    .setMaxNumberOfClients(5)
                        .build()
                } else {
